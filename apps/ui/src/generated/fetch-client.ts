...
export function extractionControllerCreate(
	{
		beanId,
		createExtractionDto
	}: {
	...
	return oazapfts.ok(
		oazapfts.fetchJson<{
			...
			`/extraction/${encodeURIComponent(beanId)}`,
			oazapfts.json({
				...opts,
				method: 'POST',
...
export function extractionControllerFindAllForBean(
	{
		beanId
	}: {
		beanId: number;
...
export function extractionControllerFindOne(
	{
		id
	}: {
		id: number;
	...
	return oazapfts.ok(
		oazapfts.fetchJson<{
			status: 200;
			data: Extraction;
		}>(`/extraction/${encodeURIComponent(id)}`, {
			...opts
		})
	);
}
export function extractionControllerUpdate(
	{
		id,
		updateExtractionDto
	}: {
...
export function extractionControllerRemove(
	{
		id
	}: {
		id: number;
	...
	return oazapfts.ok(
		oazapfts.fetchText(`/extraction/${encodeURIComponent(id)}`, {
			...opts,
			method: 'DELETE'
		})
	);
}
export function beanControllerCreate(
	{
		createBeanDto
	}: {
...
export function beanControllerFindOne(
	{
		id
	}: {
		id: string;
	...
	return oazapfts.ok(
		oazapfts.fetchJson<{
			status: 200;
			data: Bean;
		}>(`/bean/${encodeURIComponent(id)}`, {
			...opts
		})
	);
}
export function beanControllerUpdate(
	{
		id,
		updateBeanDto
	}: {
...
export function beanControllerRemove(
	{
		id
	}: {
		id: string;
	...
	return oazapfts.ok(
		oazapfts.fetchText(`/bean/${encodeURIComponent(id)}`, {
			...opts,
			method: 'DELETE'
		})
	);
...
export function groupControllerRemove(
	...
	return oazapfts.ok(
		oazapfts.fetchText(`/group/${encodeURIComponent(id)}`, {
			...opts,
			method: 'DELETE'
		})
	);
